...
import { detailsUser, upadateUserProfile } from '../action/UserAction';
...
function ProfileScreen(props) {
    ...
    const {success: successUpdate, error: errorUpadte,loading: loadingUpadte} = userUpdateProfile;
    useEffect(() => {
        if(!user){
                ...
                <div>
                    <h1>User Profile</h1>
                </div>
                        {
                            ...
                            :(
                            <>
                            {loadingUpadte && <LoadingBox></LoadingBox>}
                            {errorUpadte && <MessageBox variant="danger">{errorUpadte}</MessageBox>}
                            {successUpdate && (
                                <MessageBox variant="success">Profile Update success</MessageBox>
                            )}
                            <div>
